...
class PesaPal(object):
    ...
    def generate_iframe_src(self):
        ...
        post_xml = "<?xml version='1.0' encoding='utf-8'?><PesapalDirectOrderInfo " \
                   ...
                   "xmlns:xsd='http://www.w3.org/2001/XMLSchema' Amount='%s' Description='%s' Type='%s' " \
                   "Reference='%s' FirstName='%s' LastName='%s' Email='%s' PhoneNumber='%s' " \
                   "xmlns='http://www.pesapal.com' />" % (str(self.amount), self.description, self.type_,
                                                          self.reference, self.first_name, self.last_name, self.email, self.phone_number)

        iframe_src = OAuth.OAuthRequest.from_consumer_and_token(consumer, self.token, "GET", self.env_url, self.params)
    ...
    def check_transaction_status(self, tracking_id, merchant_reference):
        ...
        request_status = OAuth.OAuthRequest.from_consumer_and_token(consumer, self.token, "GET", self.status_url, self.params)
