...
class OAuthToken(object):
    ...
    def to_string(self):
        ...
        OAuthUtil.urlencode_rfc3986(self.key), OAuthUtil.urlencode_rfc3986(self.secret))
...
class OAuthSignatureMethod_HMAC_SHA1(OAuthSignatureMethod):
    ...
    def build_signature(self, request, consumer, token):
        ...
        print(hashed.digest())
        print(base64.encodebytes(hashed.digest()).decode('utf-8'))
...
class OAuthRequest(object):
    ...
    def from_consumer_and_token(consumer, token, http_method, http_url, parameters=None):
        ...
        parameters = utils.merge_two_dicts(defaults, parameters)

        return OAuthRequest(http_method, http_url, parameters)

    def set_parameter(self, name, value, allow_duplicates=True):

        if (allow_duplicates == True and name in self.parameters):
...
class OAuthUtil(object):
    ...
    def urlencode_rfc3986(input):
        if (isinstance(input, list) == True):
            ...
            raw_encoded_string = quote(str(input), safe='').replace("%7E", " ")
